...
public class Car {
    ...
    public static void main(String[] args) {
        Vector weights = getRandomTheta(7);
        int numSteps = 70000;
        double learningRate = 0.0000001;
        System.out.println("weights: " + weights);
        System.out.println("Training loss: " + MSE(trainingData, weights));
        System.out.println("Test loss: " + MSE(testingData, weights));
        // gradient descent in between
        for(int i= 0; i < numSteps; i++){
            Vector gradient = getGradient(weights, trainingData);
            weights = weights.subtract(gradient.scale(learningRate));
            if(i%1000 == 0){
    ...
    public double predict(Vector weight, Vector input){
       return weight.dot(input);
   }
    
    
    public static Vector getGradient(Vector weights, Car[] dataSet){
        Vector sum = new Vector(7);
        for(Car car: dataSet){
            double difference = weights.dot(car.getAttributes(true)) - car.mpg;
            sum = sum.add(car.getAttributes(true).scale(difference));
        }
        return sum.scale(1.0/dataSet.length);
    }
    
    
   
   public static double MSE(Car[] dataSet, Vector weights){
       double sum = 0;
       for(Car car: dataSet){
          sum += Math.pow((weights.dot(car.getAttributes(true)) - car.mpg),2);
       }
    ...
    public Vector getAttributes(boolean constant) {
        Vector result = new Vector(Attribute.count + 1);
        result.set(0, 1.0);
        for (Attribute attribute : Attribute.values()) {
            result.set(attribute.ordinal(), this.get(attribute));
        }
    ...
    public static Vector getRandomTheta(int dimensions){
